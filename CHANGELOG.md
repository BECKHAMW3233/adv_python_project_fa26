...
the pending, not-yet-committed work from entries 27-32) actually works correctly end to
end against the real supplied source data, not just the unit test suite.

- Ran the full pytest suite (72 tests, all passing) and a forced --refresh reconversion
  against the real source files (0 warnings, 0 errors across all three importers: 224
  MOS records, 257 training records, 1230 program requirement records).
- Ran the interactive CLI end to end via piped input against real data multiple times
  (MOS 12P skill level 30, MOS 25B skill level 10), confirming the credit profile, ranked
  recommendations, and exported report file all match; separately confirmed the 'exit'
  and 'back' navigation keywords behave correctly (clean exit with no report generated;
  back correctly re-prompts and the flow still completes).
- Ran the project's existing exhaustive combination sweep (_full_sweep_test.py): 4,621
  runs across every real MOS/skill-level/training combination, 0 exceptions, 0 suspicious
  values.
- Specifically verified the pick-group credit-capping behavior (the design decided in
  entry 17) against real data: swept all 32 real MOS/skill-level combinations, found 5
  real cases where more than one held course satisfies the same pick-group, and confirmed
  in each case the engine fills the group's own stated credit target using distinct
  courses at their full value (highest first) and reports the remainder as surplus rather
  than summing everything. Separately confirmed across all 83 real recommendations
  produced that no course is ever counted more than once within the same program.
  Presented this design choice back to William for explicit reconfirmation, since a pool
  with a credit target larger than one course's value can legitimately require more than
  one course to fill it -- confirmed to keep the existing behavior as-is.
- Verified that a course a veteran receives credit for, but that matches no FTCC program
  at all, still appears in the potential-credit summary (console output and exported
  report). Confirmed with a real example: MOS 25B skill level 10 grants credit for
  CJC217, which matches none of the 10 FTCC programs, and it still appears in both the
  console output and the exported report's credit table. 16 of the 32 real MOS/skill-level
  combinations include at least one such course.
- No code changes resulted from this pass -- every check confirmed already-implemented
...
## [32] 2026-09-08 — Remove all remaining backtick inline-code spans from every .md file; add Markdown formatting rule to CLAUDE.md

**Why:** per William's direction -- [31] only removed backticks around the three data-path
references in INSTRUCTIONS.md, but William confirmed the invisible-text rendering bug
affects every backtick-delimited inline code span, not just those three, and it was showing
up in every .md file in the project. William also pointed out this backtick convention was
never requested -- Claude Code introduced it unprompted across earlier sessions -- and asked
for a standing CLAUDE.md rule to prevent it from being reintroduced.

- Stripped every single-backtick inline code span from CLAUDE.md, README.md,
  INSTRUCTIONS.md, and CHANGELOG.md (including this file's own historical entries),
  leaving the surrounding text unchanged. Triple-backtick fenced code blocks (the shaded
  command-example boxes in README.md and INSTRUCTIONS.md) were left alone -- confirmed with
  William this is a full-block rendering, not the inline invisible-text bug, and the rule
  added below only covers inline spans.
- Added a "Markdown formatting" section to CLAUDE.md banning backtick-delimited inline code
  spans in any .md file in this project going forward, with an incident note explaining why.
- Built and diffed stripped versions as CLAUDE_temp.md, README_temp.md,
  INSTRUCTIONS_temp.md, and CHANGELOG_temp.md first, per the project's temp-file convention,
  before promoting the changes into the real files.

**Files changed:** CLAUDE.md, README.md, INSTRUCTIONS.md, CHANGELOG.md

---

## [31] 2026-09-08 — Remove backtick formatting around data-path references in INSTRUCTIONS.md

**Why:** per William's direction -- backtick inline-code formatting around source_data/ and
normalized_data/ in INSTRUCTIONS.md was rendering as invisible text in the markdown viewer
William uses to read the file.

- Removed the backtick pair around each of the three occurrences that name a path containing
  "data" (source_data/ on the first reference, normalized_data/ on the next two), leaving
  the surrounding text and every other backticked term (conversion_issues/,
  student_reports/, logs/app.log, etc.) unchanged.

**Files changed:** INSTRUCTIONS.md

---

## [30] 2026-09-06 — Add INSTRUCTIONS.md

**Why:** per William's direction -- a full CLI usage guide plus an explanation of the skill-level/rank structure, for any user of the program, not just someone reading the code or CHANGELOG.

- Added INSTRUCTIONS.md: a walkthrough of running the program (first run vs. --refresh, the four-step interactive flow, back/exit navigation, where output files land) and a skill-level <-> Army rank reference table (SL1-SL6 and the full E-1 through E-9 pay grade scale), citing DA Pam 611-21, Chapter 10 Section C (api.army.mil) as the source William supplied and verified against official content. Deliberately documentation only -- no code reads or displays this table live; William explicitly decided against building a live rank-reference feature for this phase, so the explanation lives here instead of in a new module.

**Files changed:** INSTRUCTIONS.md (new)

---

## [29] 2026-09-06 — Show real course titles in the CLI, exported report, and normalized data

**Why:** per William's direction -- seeing only a bare course code (e.g. "CJC120") in the console output or exported report doesn't tell a user what the course actually is. The underlying data gap (a real title sitting in a workbook column/text position the importer already read past) was first noticed while building a course-title display into the GUI referenced in [27]/[28]; William then asked for this repository's own CLI and exported report to show titles too.

- Confirmed against the real 2026_POS_Reduced.xlsx before changing anything: structured rows carry a real course title in a dedicated column (column 25) that the importer was reading everything around but never itself; flattened (non-structured) rows have the title embedded in the row's text blob, between the term code and the class/lab/clinic/credit numbers, already inside what _FLATTENED_COURSE_PATTERN was matching (as an unnamed, discarded .*?).
- models.py: added course_title: str = "" (default, so no existing positional ProgramRequirement/MatchedCourse/CreditProfileEntry construction anywhere -- including test fixtures -- needed to change) to ProgramRequirement, MatchedCourse, and CreditProfileEntry.
- importers/program_workbook_importer.py: named the previously-anonymous title group in _FLATTENED_COURSE_PATTERN; added a _TITLE_COLUMN = 25 constant and read it for structured rows. Both paths populate the new course_title field.
- services/recommendation_engine.py: _to_matched_courses() now carries course_title through from the source ProgramRequirement into the MatchedCourse objects the CLI and report display.
- services/credit_evaluator.py: build_credit_profile() gained an optional program_records parameter, used as a fallback title source -- an MOS-sourced course already has its own title (MOSCourseEquivalency.course_title), but a training-sourced course doesn't (TrainingEquivalency has no title field), so its title is looked up from the FTCC program catalog when available, and left blank (not guessed) when no source names it anywhere. main.py updated to pass program_records through.
- cli.py: print_credit_profile() and print_recommendations() now show CODE (Title) instead of a bare code.
- reports/report_generator.py: build_report_text()'s two course tables (Potential Credit Summary, Matched Courses per program) gained a Title column.
- Tests: extended test_program_workbook_importer.py's existing structured/flattened-text test with a title assertion for both paths; added 3 new tests to test_credit_evaluator.py covering the MOS-title case, the training-falls-back-to-catalog case, and the no-source-names-it-blank case.

Verified: full test suite passes (72 tests total: 63 existing + 6 from [28] + 3 new here; 1 existing test extended in place rather than duplicated). Re-ran the real conversion against the actual supplied source files: 1199 of 1230 program requirement rows now have a non-empty course_title. Ran a real end-to-end CLI session (MOS 12P, skill level 10, no trainings) via piped stdin and confirmed titles appear correctly in both the console output and the exported report file for every course shown (e.g. ELC112 (DC/AC Electricity)); confirmed the training-catalog-fallback path with a direct unit test (BUS234 via "SHARP certification" correctly resolving to "Sexual Assault Prevention" from the program catalog) rather than a live run, since no MOS/training combination tried manually happened to exercise it.

**Files changed:** models.py, importers/program_workbook_importer.py, services/recommendation_engine.py, services/credit_evaluator.py, main.py, cli.py, reports/report_generator.py, tests/test_program_workbook_importer.py, tests/test_credit_evaluator.py

---

## [28] 2026-09-06 — Add course-credit reference resolver

**Why:** per William's direction, continuing the same GUI-based debugging and error checking described in [27]'s Why. Selecting certain military schools (e.g. Army Basic Leader Course, Warrior Leader Course) in that GUI produced zero visible credit even though the source document names real FTCC courses for them. Investigating confirmed this wasn't a parsing bug: the source only gives one combined hours total for multiple named courses (e.g. "COM 120, HUM 230" -- "6" hours, no per-course split), so the app was correctly declining to invent a split, but the result was that named courses vanished from the output with no visible trace.

- Added services/course_credit_resolver.py: build_reference() builds a course_id -> credits map from two confirmed sources only -- the FTCC program catalog (program_requirements.csv, preferred, since it's an official FTCC-assigned value) and any other training row that already resolved the same course unambiguously on its own (used only when the catalog doesn't mention the course). Confirmed against the real data first that no course_id carries conflicting credit values across FTCC programs (0 conflicts across 551 course codes), so preferring the catalog is a safe, deterministic choice, not an arbitrary tie-break. resolve_training_credits() then fills in course_credits for any training row still marked credits_unresolved whose course is in the reference, preserving the original combined-hours figure in its notes; a course found nowhere else is left exactly as the importer produced it, still flagged for manual verification -- never resolved by arithmetic (e.g. splitting the total evenly across courses), which was considered and rejected: checked against real data, an even split would have been factually wrong in at least one case (Army Basic Training: 7 hours / 4 courses = 1.75 credits each by division, but PED110's real, catalog-confirmed value is 2).
- models.py: added CourseCreditReference (course_id, credits, source).
- pipeline.py: convert() now builds the reference after both the training and program imports complete, writes it to a new normalized_data/course_credits_reference.csv, and resolves the training records against it before writing training_equivalencies.csv (moved that write to after resolution). print_summary() reports how many training rows were resolved via the reference vs. remain genuinely unresolved.
- Added tests/test_course_credit_resolver.py (6 tests): catalog preferred over a training row, training-row fallback when the catalog doesn't mention the course, a course excluded from the reference when confirmed nowhere, resolution filling in a previously-unresolved row correctly, an unmatched row left untouched, and an already-ok row never overwritten by the reference.

Verified: full test suite passes (69 tests at this point: 63 existing + 6 new, no changes needed to any existing test). Re-ran the real conversion against the actual supplied source files: pipeline.print_summary() reported 161 of 196 previously-ambiguous training rows now resolve to a real, catalog- or cross-reference-confirmed credit value, with 35 remaining honestly unresolved; ConversionValidator reports 0 warnings and 0 errors against the full real dataset. The specific per-row breakdown (e.g. that Army Basic Leader Course's COM120 and HUM230 both resolve to 3 credits each, and that PED172/PED169/HEA112 from Army Basic Training have no confirmed value anywhere in any of the three supplied source files) was established during the original investigation in the prototyping copy referenced above, and reproduced here via the same aggregate counts on this repository's own copy of the source data.

**Files changed:** services/course_credit_resolver.py (new), models.py, pipeline.py, tests/test_course_credit_resolver.py (new), normalized_data/training_equivalencies.csv, normalized_data/course_credits_reference.csv (new)

---

## [27] 2026-09-06 — Fix program importer misclassifying choice-pool courses as major_required

**Why:** per William's direction. William built a GUI in a separate copy of this program and used it to help with debugging and error checking with a GUI in place, as well as testing the GUI itself against the existing data. Working with a program's full requirement list there, it looked like one flat "required" block instead of a proper required-vs-choice breakdown. Investigating turned up a real bug, not a display problem -- raw_rule_text on several "major_required" rows literally said "Take 9 credits From..." or "Take 18 credits From...", which is a choice-pool instruction, not a flat requirement. Only the resulting fix below, verified against this project's own real data, was brought into this repository -- the GUI itself stays in that separate copy; this project remains CLI-only per the assignment's Restrictions.

- Root cause 1: _SUBGROUP_LABEL_PATTERN (importers/program_workbook_importer.py) couldn't match a label like "MAJ OTHER REQ. PICK" because the period between "REQ" and "PICK" broke the regex's character class, so the label was never recognized as a choice-pool marker at all. Fixed by allowing a period in the label only when it's *not* part of a decimal number (\.(?!\d)), so it doesn't also start swallowing decimal hour/credit values from adjacent blob text -- an initial looser fix (allowing any period) was caught by re-diffing against real data before shipping it in the prototyping copy: it corrupted several already-correct labels by pulling in text like "3.00 0.00 0.00 3.00" from nearby course-hour cells.
- Root cause 2: a "group" event (a new top-level section starting) never reset the parser's subgroup state, so a group with no subgroup marker of its own could silently inherit whatever subgroup_type/label the *previous*, unrelated group last set -- sometimes accidentally landing on "major_required", sometimes on a stale choice-group label. Fixed by resetting subgroup state on every new group event.
- A related pattern -- a group with a "Take N credits From..." instruction but no "...Pick"/"...Courses"-suffixed label anywhere (e.g. "Other Major Requirements MAJ OTHER REQUIRED > Take 9 credits") -- was deliberately *not* auto-classified as a choice pool: without a real label there's no safe way to tell whether it's one pool or several unrelated ones sharing the same parent group (confirmed this risk directly in the prototyping copy: an earlier attempt that guessed a shared fallback label merged two real, differently-sized pick pools in one program into one, which would have corrupted its credit cap). These now correctly land in requirement_type="unresolved" / status="manual_review", consistent with the spec's "don't guess ambiguous structure" rule, rather than either the old wrong guess or a new, differently-wrong guess.
- Added a status="manual_review" + explanatory note for any row that resolves to requirement_type="unresolved" via a course event, for consistency with how every other unresolved case in this codebase is represented.

Verified: full test suite passes (63 tests, no regressions -- this fix alone required no test changes; see [28]/[29] for tests added by later, related work). Re-ran the real conversion against this repository's own copy of the actual supplied source files: ConversionValidator reports 0 warnings and 0 errors against the full real dataset, matching the result before the fix. The specific row-level impact (49 rows across 5 programs corrected from major_required to major_choice; 71 rows across 6 programs moved from an incorrectly-confident classification to honest unresolved/manual_review) was measured via a full before/after diff of all 1230 program requirement rows during the original investigation in the prototyping copy referenced above; this repository's own conversion was re-run after applying the same fix and produces the same 0-warning/0-error result, but was not independently re-diffed row-by-row here.

**Files changed:** importers/program_workbook_importer.py
